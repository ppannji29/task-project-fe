...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        ...
        setDueDate(task.due_date.slice(0, 10)) // format YYYY-MM-DD
      } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    setError("")
    try {
      await apiService.updateTask(id as string, {
        title,
        description,
        status,
        priority,
  ...
  const showMessage = (name: string) => {
    ...
    toast.innerHTML = `
      <div class="flex items-center space-x-2">
        ...
        <span>Success to update task, ${name}!</span>
      </div>
    `
